...
kraken_file_to_df <- function(fname){
    ...
    #     }
    df$name <- ifelse(trimws(df$name)=="", df$taxid, df$name)
    return(df)
}
...
make_gct_from_kraken <- function(merge.mat, sample.metadata, tax.array){
    ...
    tax.array.subset <- tax.array[match(common.taxids, tax.array$taxid),]
    kgct <- new('GCT')
    kgct@rdesc <- tax.array.subset
    rownames(kgct@rdesc) <- tax.array.subset$id
    kgct@rid <- tax.array.subset$id
    kgct@mat <- merge.mat[match(common.taxids, rownames(merge.mat)),,drop=FALSE]
...
normalize_kgct <- function(kgct, min.frac = 0.001, check_filtered=TRUE){
    ...
    # remove samples with 0 read classified 
    incl <- kgct@cid[!apply(kgct@mat, 2, function(x) sum(x)==0)]
    kgct <- subset_gct(kgct, cid=incl)
    ...
    kgct <- subset_gct(kgct, rid=keep.rows)
